...
首次配置时，在推理服务列表中选择 `荆华密态 SaaS`（Provider ID：`jinghua_saas`），然后输入你自己的 SaaS API Key。配置向导会把 Key 加密保存到 SClaw 现有 Secret Store；不要把 Key 写进仓库、启动参数或安装包。
...
开发环境也可以通过 `LLM_BACKEND=jinghua_saas` 选择该 Provider，并通过现有配置向导或 `JINGHUA_API_KEY` 提供凭据。环境变量仅用于本机开发，不应写进 `.env` 示例或提交到 Git。
...
- `target/SClaw.app`
  作为最终生成的 app
- `target/SClaw-local-unsigned.dmg`
  作为本地未签名 dmg
...
准备经过批准的 Node.js v24.14.0 arm64 Mach-O 可执行文件，并通过 `SCLAW_NODE_BINARY` 明确传入。该变量只在制作 DMG 的电脑上使用；Node 会被复制进 `SClaw.app`，最终用户不需要安装 Node 或配置这个变量。

```bash
SCLAW_NODE_BINARY=/absolute/path/to/approved/node \
...
2. 删除旧的 `target/SClaw.dmg` 和 `target/SClaw-local-unsigned.dmg`
3. 复制 `assets/SClaw.app` 到 `target/SClaw.app`
4. 复制 `target/release/ironclaw` 到 `target/SClaw.app/Contents/MacOS/ironclaw`
5. 校验并复制 Node、加密 sidecar 和固定版本 JSSDK 到 App Resources
6. 生成 `target/SClaw-local-unsigned.dmg`

不带 `--release` 的模式用于本机开发验证，生成未签名的 `target/SClaw-local-unsigned.dmg`，不要把它交给最终用户安装。两种模式都必须在打包机上显式提供 `SCLAW_NODE_BINARY`；输入文件必须位于 `target/SClaw.app` 之外，脚本也会拒绝错误版本、非 arm64、不可执行或最低 macOS 版本高于 App 声明的 Node。
...
脚本启动时会先删除旧的本地与正式 DMG，再校验 arm64 Mach-O、Node v24.14.0、macOS `minos`、JSSDK 1.0.15 和 SHA256 清单。正式模式会单独签名内置 Node，随后依次完成 App Hardened Runtime 签名、App 验签、DMG 签名、Apple 公证、staple、Gatekeeper 检查和 DMG 完整性验证。任何一步失败都会以非零状态退出，并且不会保留可能被误认为本次输出的旧 DMG。

### 打包结果

两种模式都会生成 `target/SClaw.app`。DMG 输出按模式区分：

- `target/SClaw-local-unsigned.dmg`：仅本机开发验证，不要分发
- `target/SClaw-0.1.3-arm64.dmg`：正式签名模式生成，最终用户应安装和分发这个文件

旧名称 `target/SClaw.dmg` 会被自动删除，避免误装陈旧的本地包。最终用户安装正式 DMG 后，直接打开 SClaw 即可；App 会使用内置 Node 启动 sidecar 和本地服务，不依赖用户电脑的 `PATH` 或外部 Node 环境。
