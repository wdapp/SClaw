...
/// They are only available when explicitly injected at build time.
...
pub fn builtin_credentials(secret_name: &str) -> Option<OAuthCredentials> {
    match secret_name {
        "google_oauth_token" => match (GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET) {
...
mod tests {
    ...
    fn test_google_returns_based_on_compile_env() {
        ...
        match (
            option_env!("IRONCLAW_GOOGLE_CLIENT_ID"),
            option_env!("IRONCLAW_GOOGLE_CLIENT_SECRET"),
        ) {
            (Some(client_id), Some(client_secret))
                if !client_id.is_empty() && !client_secret.is_empty() =>
            {
                let creds = creds.expect("compile-time credentials should be present");
                assert_eq!(creds.client_id, client_id);
                assert_eq!(creds.client_secret, client_secret);
            }
            _ => assert!(
                creds.is_none(),
                "google built-in credentials should be absent without compile-time env vars"
            ),
        }
    }
