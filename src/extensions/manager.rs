...
use crate::channels::{ChannelManager, FeishuChannel, FeishuChannelConfig, OutgoingResponse};
...
const EMBEDDED_FEISHU_CAPABILITIES: &[u8] =
    include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/channels-src/feishu/feishu.capabilities.json"));
const NATIVE_CHANNEL_PLACEHOLDER_WASM: &[u8] = b"native-channel-placeholder\n";
...
fn channel_auth_instructions(
    ...
    if channel_name == TELEGRAM_CHANNEL_NAME && secret.name == "telegram_bot_token" {
        return format!(
            "{} After you submit it, SClaw will show a one-time verification code. Send `/start CODE` to your bot in Telegram and SClaw will finish setup automatically.",
            secret.prompt
        );
    }
...
fn telegram_verification_instructions(bot_username: Option<&str>, code: &str) -> String {
    if let Some(username) = bot_username.filter(|username| !username.trim().is_empty()) {
        return format!(
            "Send `/start {code}` to @{username} in Telegram. SClaw will finish setup automatically."
        );
    }

    format!("Send `/start {code}` to your Telegram bot. SClaw will finish setup automatically.")
}
...
impl ExtensionManager {
    ...
    async fn install_from_entry(
        ...
        if entry.kind == ExtensionKind::WasmChannel && entry.name == FEISHU_CHANNEL_NAME {
            return self.install_native_feishu_channel().await;
        }
    ...
    async fn install_wasm_from_buildable(
        ...
        // Resolve source directory (where `cargo component build` should run)
        let resolved_source_dir = source_dir.map(resolve_dir);

        // Resolve build directory (where artifacts are expected to land)
        let resolved_dir = match build_dir {
            Some(dir) => {
                resolve_dir(dir)
            }
            None => resolved_source_dir
                .clone()
        ...
        let wasm_src = if let Some(existing) =
            crate::registry::artifacts::find_wasm_artifact(&resolved_dir, binary_name, "release")
        {
            existing
        } else if let Some(build_root) = resolved_source_dir.as_ref() {
            crate::registry::artifacts::build_wasm_component(build_root, binary_name, true)
                .await
                .map_err(|e| {
    ...
    async fn install_native_feishu_channel(&self) -> Result<InstallResult, ExtensionError> {
        ...
        let wasm_path = self
            .wasm_channels_dir
            .join(format!("{}.wasm", FEISHU_CHANNEL_NAME));
        let cap_path = self
            .wasm_channels_dir
            .join(format!("{}.capabilities.json", FEISHU_CHANNEL_NAME));

        if wasm_path.exists() || cap_path.exists() {
            return Err(ExtensionError::AlreadyInstalled(
                FEISHU_CHANNEL_NAME.to_string(),
            ));
        }

        tokio::fs::write(&wasm_path, NATIVE_CHANNEL_PLACEHOLDER_WASM)
            .await
            .map_err(|e| ExtensionError::InstallFailed(e.to_string()))?;
        tokio::fs::write(&cap_path, EMBEDDED_FEISHU_CAPABILITIES)
            .await
            .map_err(|e| ExtensionError::InstallFailed(e.to_string()))?;

        tracing::info!(
            channel = FEISHU_CHANNEL_NAME,
            "Installed native Feishu channel from embedded capabilities"
        );

        Ok(InstallResult {
            name: FEISHU_CHANNEL_NAME.to_string(),
            kind: ExtensionKind::WasmChannel,
            message: "Feishu channel installed from embedded native runtime. Run activate to load it."
                .to_string(),
        })
    }
    ...
    async fn activate_wasm_channel(&self, name: &str) -> Result<ActivateResult, ExtensionError> {
        ...
        if name == FEISHU_CHANNEL_NAME {
            let cap_path = self.wasm_channels_dir.join(format!("{}.capabilities.json", name));
            return self
                .activate_native_feishu_channel(name, &channel_manager, &cap_path)
                .await;
        }
    ...
    async fn activate_native_feishu_channel(
        &self,
        name: &str,
        channel_manager: &Arc<ChannelManager>,
        cap_path: &std::path::Path,
    ) -> Result<ActivateResult, ExtensionError> {
        let config = self.load_native_feishu_config(cap_path).await?;
        ...
        tracing::info!(channel = %name, "Activated native Feishu channel in persistent WebSocket mode");
        ...
        Ok(ActivateResult {
            ...
            tools_loaded: Vec::new(),
            message: format!(
                "Channel '{}' activated in persistent WebSocket mode",
                name
            ),
        })
    }

    async fn load_native_feishu_config(
        &self,
        cap_path: &std::path::Path,
    ) -> Result<FeishuChannelConfig, ExtensionError> {
        let bytes = tokio::fs::read(cap_path)
            .await
            .map_err(|e| ExtensionError::ActivationFailed(e.to_string()))?;
        let cap_file = crate::channels::wasm::ChannelCapabilitiesFile::from_bytes(&bytes)
            .map_err(|e| ExtensionError::ActivationFailed(e.to_string()))?;

        let mut config_value = serde_json::to_value(&cap_file.config)
            .map_err(|e| ExtensionError::ActivationFailed(e.to_string()))?;
        let app_id = self
            .secrets
            .get_decrypted(&self.user_id, "feishu_app_id")
            .await
            .map_err(|e| {
                ExtensionError::ActivationFailed(format!("Missing feishu_app_id secret: {e}"))
            })?
            .expose()
            .to_string();
        let app_secret = self
            .secrets
            .get_decrypted(&self.user_id, "feishu_app_secret")
            .await
            .map_err(|e| {
                ExtensionError::ActivationFailed(format!("Missing feishu_app_secret secret: {e}"))
            })?
            .expose()
            .to_string();

        if let Some(object) = config_value.as_object_mut() {
            object.insert("app_id".to_string(), serde_json::Value::String(app_id));
            object.insert(
        ...
        let config: FeishuChannelConfig = serde_json::from_value(config_value)
            .map_err(|e| ExtensionError::ActivationFailed(e.to_string()))?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_install_buildable_channel_uses_source_dir_when_build_dir_missing() {
        let dir = tempfile::tempdir().expect("temp dir");
        let source_dir = dir.path().join("channels-src").join("feishu-like");
        let target_artifact_dir = source_dir.join("target/wasm32-wasip2/release");
        std::fs::create_dir_all(&target_artifact_dir).expect("artifact dir");
        std::fs::write(
            target_artifact_dir.join("feishu_like_channel.wasm"),
            b"\0asm fake",
        )
        .expect("fake wasm");
        std::fs::write(
            source_dir.join("feishu-like.capabilities.json"),
            serde_json::json!({
                "type": "channel",
                "name": "feishu-like",
        ...
        manager
            .inject_registry_entry(crate::extensions::RegistryEntry {
                name: "feishu-like".to_string(),
                display_name: "Feishu Like".to_string(),
                kind: ExtensionKind::WasmChannel,
                description: "test buildable channel".to_string(),
                keywords: vec!["feishu".to_string()],
                source: ExtensionSource::WasmBuildable {
                    source_dir: source_dir.display().to_string(),
                    build_dir: None,
                    crate_name: Some("feishu-like-channel".to_string()),
                },
        ...
        let result = manager
            .install("feishu-like", None, Some(ExtensionKind::WasmChannel))
            .await
            .expect("install should succeed from source_dir artifacts");

        assert!(result.message.contains("installed"));
        assert!(
            dir.path().join("channels/feishu-like.wasm").exists(),
            "channel wasm should be installed into target channels dir"
        );
        assert!(
            dir.path()
                .join("channels/feishu-like.capabilities.json")
                .exists(),
            "channel capabilities should be installed into target channels dir"
        );
    }

    #[tokio::test]
    async fn test_install_feishu_channel_uses_embedded_native_runtime_files() {
        let dir = tempfile::tempdir().expect("temp dir");
        let manager =
            make_manager_custom_dirs(dir.path().join("tools"), dir.path().join("channels"));
        ...
        manager
            .inject_registry_entry(crate::extensions::RegistryEntry {
                name: super::FEISHU_CHANNEL_NAME.to_string(),
                display_name: "Feishu / Lark Channel".to_string(),
                kind: ExtensionKind::WasmChannel,
                description: "native Feishu channel".to_string(),
                keywords: vec!["feishu".to_string()],
                source: ExtensionSource::WasmBuildable {
                    source_dir: "/nonexistent/channels-src/feishu".to_string(),
                    build_dir: None,
                    crate_name: Some("feishu-channel".to_string()),
                },
                fallback_source: None,
                auth_hint: AuthHint::CapabilitiesAuth,
        ...
        let result = manager
            .install(
                super::FEISHU_CHANNEL_NAME,
                None,
                Some(ExtensionKind::WasmChannel),
            )
            .await
            .expect("feishu install should succeed without cargo-component");

        assert!(result.message.contains("embedded native runtime"));

        let installed_wasm = dir.path().join("channels/feishu.wasm");
        let installed_caps = dir.path().join("channels/feishu.capabilities.json");
        assert!(installed_wasm.exists(), "placeholder wasm should be installed");
        assert!(
            installed_caps.exists(),
            "embedded feishu capabilities should be installed"
        );

        let caps_bytes = tokio::fs::read(installed_caps).await.expect("read caps");
        assert_eq!(
            caps_bytes,
            super::EMBEDDED_FEISHU_CAPABILITIES,
            "installed capabilities should match embedded copy"
        );
    }
