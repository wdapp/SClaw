...
impl SetupWizard {
    ...
    pub async fn run(&mut self) -> Result<(), SetupError> {
        print_banner();
        print_header("SClaw Setup Wizard");
    ...
    async fn step_channels(&mut self) -> Result<(), SetupError> {
        ...
        if selected.contains(&CHANNEL_INDEX_HTTP) {
            ...
            if let Some(ref ctx) = secrets {
                ...
                self.settings.channels.http_port = Some(crate::config::DEFAULT_HTTP_PORT);
                print_info("HTTP webhook enabled on port 18080 (set HTTP_WEBHOOK_SECRET in env)");
            }
        } else {
