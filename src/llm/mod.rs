        ...
        let http_client =
            reqwest::Client::builder()
                ...
                .map_err(|e| LlmError::RequestFailed {
                    ...
                    reason: format!("Failed to create insecure TLS client: {e}"),
                })?;
        builder = builder.http_client(http_client);
        tracing::warn!(
            provider = %config.provider_id,
            "LLM_INSECURE_TLS is enabled; TLS certificate verification is disabled for this provider"
        );
    }
