...
fn is_host_managed_native_channel(name: &str) -> bool {
    name == "feishu"
}
...
impl WasmChannelLoader {
    ...
    pub async fn load_from_dir(&self, dir: &Path) -> Result<LoadResults, WasmChannelError> {
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            if is_host_managed_native_channel(&name) {
                tracing::debug!(
                    name = name,
                    path = %path.display(),
                    "Skipping host-managed native channel placeholder during WASM startup load"
                );
                continue;
            }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_load_from_dir_skips_host_managed_native_channel_placeholder() {
        ...
        let dir = TempDir::new().unwrap();
        std::fs::File::create(dir.path().join("feishu.wasm")).unwrap();
        let mut cap_file =
            std::fs::File::create(dir.path().join("feishu.capabilities.json")).unwrap();
        cap_file.write_all(b"{}").unwrap();

        let results = loader.load_from_dir(dir.path()).await.unwrap();
