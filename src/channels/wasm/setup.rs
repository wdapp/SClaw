...
async fn inject_channel_secrets_into_config(
    ...
    for &(config_key, secret_name) in secret_config_mappings {
        match secrets.get_decrypted(owner_scope_id, secret_name).await {
            Ok(decrypted) => {
                config_updates.insert(
...
mod tests {
    ...
    use super::inject_channel_secrets_into_config;

    fn test_secrets_store() -> Arc<dyn SecretsStore + Send + Sync> {
        let crypto = Arc::new(
            SecretsCrypto::new(secrecy::SecretString::from("a".repeat(32)))
                .expect("test master key should be valid"),
        );
        Arc::new(InMemorySecretsStore::new(crypto))
    }

    #[tokio::test]
    async fn feishu_config_injection_uses_provided_owner_scope() {
        let secrets = test_secrets_store();
        secrets
            .create(
                "owner-a",
                CreateSecretParams::new("feishu_app_id", "cli_test_app_id"),
            )
            .await
            .expect("app id secret should be created");
        secrets
            .create(
                "owner-a",
                CreateSecretParams::new("feishu_app_secret", "test_app_secret"),
            )
            .await
            .expect("app secret should be created");

        let mut config_updates = HashMap::new();
        let secrets_store = Some(secrets);
        ...
        assert_eq!(
            ...
            Some(&serde_json::json!("cli_test_app_id"))
        );
        assert_eq!(
            config_updates.get("app_secret"),
            Some(&serde_json::json!("test_app_secret"))
        );
    }

    #[tokio::test]
    async fn feishu_config_injection_does_not_fall_back_to_default_scope() {
        let secrets = test_secrets_store();
        secrets
            .create(
                "default",
                CreateSecretParams::new("feishu_app_id", "cli_default_only"),
            )
            .await
            .expect("default-scope secret should be created");
        ...
        assert!(
            !config_updates.contains_key("app_id"),
            "Feishu config injection should not read secrets from the default scope"
        );
    }
}
